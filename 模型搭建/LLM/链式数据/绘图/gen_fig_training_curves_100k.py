# -*- coding: utf-8 -*-
"""Fig: 100k 复刻 逐轮训练曲线 (val AUC, 3种子 mean + min-max 带)。
数据源: 模型/chain_airseq_100k/history_airseq_ca_{mode}_s{seed}.csv
输出: 绘图/fig_training_curves_100k.pdf/.png (每模式一条曲线 + 种子区间带)
"""
...
def main():
    ...
    for ax, metric, ylabel in zip(
        axes, ("val_auc", "val_pr_auc"), ("Validation AUC", "Validation PR-AUC")
    ):
        for mode, label in MODE_LABELS.items():
            ...
            if not frames:
                ...
                continue
            df = pd.concat(frames)
            g = df.groupby("epoch")[metric].agg(["mean", "min", "max"]).reset_index()
        ...
        ax.set_xlim(0.8, 5.2)   # 截到有效区间, 去掉过拟合下滑尾巴
        ax.set_xticks([1, 2, 3, 4, 5])
        ax.legend(frameon=False, loc="lower left")
    save(fig, "fig_training_curves_100k")
    print("Saved: fig_training_curves_100k")
